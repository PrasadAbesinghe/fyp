...
The architecture is divided into four functional layers so as to provide a secure and scalable vehicular network \cite{karof2014}.

\mypar
\textbf{Data Plane (Vehicular Layer)}
...
\mypar
\textbf{Blockchain Plane (Trust Layer)}

\mypar
The plane is dedicated, which offers a decentralized security basis:

\begin{itemize}
	\item \textbf{Blockchain Ledger: }Serves as a non-modifiable, non-repudiable log of node packet forwarding actions to hold the organization accountable and to avoid the problem of attackers falsifying their logs. The ledger is independent of the analysis logic by isolating this to its own plane.
\end{itemize}

\mypar
\textbf{Knowledge and Security Plane (Intelligence Layer)}

\mypar
The analytical intelligence of the framework is in this plane:

\begin{itemize}
	\item \textbf{LLM Agent: }Intelligently detects threats by forecasting and preventing the occurrence of the Grey Hole attacks by analyzing logs and reasoning semantically the importance of packets in real time.
	\item \textbf{Federated Learning Aggregator: }Federated Learning Aggregator: Aggregates the local vehicle model improvements across the globe to update the LLM without sacrificing privacy of the data \cite{yurdem2024}.
\end{itemize}
