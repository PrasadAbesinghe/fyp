...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{Figures/figure1_architecture_diagram}
	\caption{SHIELD-GH four-layer architecture: Intelligence/Knowledge plane (top), Blockchain/Trust plane, SDN Control plane, and Data/Vehicular plane (bottom). Threat verdicts bypass the untrusted SDN controller and are enforced directly via DEBSC smart contracts on the Hyperledger Fabric ledger.}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{Figures/figure2_dual_mode_architecture}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{Figures/pqc_lkh}
	\caption{PQC-LKH tree for group re-keying after vehicle isolation. Isolating attacker $V_3$ (red leaf) requires refreshing only $\lceil\log_2 8\rceil = 3$ nodes along the path to the root, compared to $N-1 = 7$ unicast KEM operations under naïve re-keying. All key pairs use Kyber-768.}
	\label{fig:pqc-lkh}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.85\textwidth]{Figures/figure_D_cryptographic_mitigation_revised}
