...
% ============================================================
% INSERT AS NEW SECTION 3.3, SHIFT EXISTING SECTIONS FORWARD
% ============================================================

\section{Cryptographic Analysis and Technique Selection}
\label{sec:crypto_analysis}

\subsection{Role of Cryptography in Grey Hole Detection and Mitigation}

A grey hole attacker receives packets through legitimate channels and selectively drops
them without altering packet content. Classical authentication (HMAC, ECDSA) verifies
packet integrity and provenance but does not constrain a node's forwarding behavior
post-receipt. Consequently, cryptography alone \emph{cannot} fully mitigate grey hole
attacks. Instead, cryptography is deployed in two phases:

\textbf{Phase 1 — Pre-Detection:} Zero-knowledge proof (ZKP)-based packet forwarding
proofs are generated by each vehicle upon receipt and forwarding of a packet. These
proofs serve as unforgeable cryptographic evidence for the blockchain-based detection
engine. A vehicle that drops packets cannot generate valid forwarding proofs, creating
a detectable discrepancy in the blockchain ledger.

\textbf{Phase 2 — Post-Detection:} Once a node is classified as malicious by the
detection engine, post-quantum cryptographic mechanisms enforce mitigation: the SDN
controller issues PQC-signed flow modification commands to isolate the node, and
revokes the node's session keys using CRYSTALS-Kyber re-keying to prevent further
network participation.

\subsection{Selected Cryptographic Techniques}

From the candidate set \{homomorphic encryption, attribute-based encryption (ABE),
broadcast encryption, homomorphic signatures, ring signatures, threshold signatures,
ZKP, multi-party computation (MPC)\} with post-quantum cryptography (PQC) mandatory,
the following are selected with justification:

\textbf{(1) Zero-Knowledge Proofs (ZKP) — Pre-detection, both planes:}
ZKP enables a vehicle to prove it forwarded a packet without revealing the packet
content or the forwarding path. Using Pedersen commitments and zk-SNARK constructions,
each vehicle generates a forwarding proof $\pi_i$ per packet batch. These proofs are
logged to the blockchain. A discrepancy between claimed and proven forwarding rates
constitutes a primary detection signal. ZKP is selected over homomorphic signatures
because it provides stronger privacy guarantees and is compatible with PQC lattice
constructions.

\textbf{(2) Threshold Signatures — Post-detection mitigation:}
A $(k, n)$-threshold signature scheme requires $k$ of $n$ trusted RSUs to co-sign
a blacklisting decision before a node is isolated. This prevents a single compromised
RSU from falsely isolating legitimate vehicles, which is critical in high-mobility
environments where RSU-level observations are limited. Selected over ring signatures
because threshold signatures provide accountability (parties are known) and are more
appropriate for infrastructure-assisted SDVN.

\textbf{(3) CRYSTALS-Kyber (KEM) — PQC, lightweight and full mitigation:}
Kyber provides IND-CCA2-secure key encapsulation under the Module-LWE hardness
assumption. It is used to establish session keys between vehicles and RSUs after
a malicious node is excluded (re-keying), and for securing blockchain transactions.
Kyber-512 is used in the lightweight mode; Kyber-768 in the full mode.

\textbf{(4) CRYSTALS-Dilithium — PQC, post-detection mitigation:}
Dilithium provides EUF-CMA-secure digital signatures under Module-LWE. It is used
to authenticate SDN controller flow modification commands and blockchain smart
contract transactions, ensuring that isolation commands cannot be forged or replayed.

\textbf{(5) Attribute-Based Encryption (ABE) — Full mode, target-specific attacks:}
For target-specific grey hole attacks in which the attacker selectively drops packets
from emergency vehicles (identified by source attribute), ABE encrypts emergency
traffic under the attribute policy \texttt{priority = EMERGENCY}. Only nodes with
valid attribute credentials can decrypt emergency packets. A grey hole attacker
without the emergency-traffic decryption key cannot perform intelligent content-aware
dropping, partially degrading the target-specific attack capability.

\textbf{Not selected:} Homomorphic encryption and MPC are excluded due to
prohibitive computational overhead on vehicular OBU hardware. Broadcast encryption
does not address the forwarding proof requirement. Homomorphic signatures provide
overlapping functionality with ZKP at higher overhead.
